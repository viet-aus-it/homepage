...
1. Before commit/PR: `pnpm run lint:fix && pnpm run typecheck` (+ `pnpm run test:run` when behaviour changed). Husky runs lint-staged on commit and lint-prepush before push.

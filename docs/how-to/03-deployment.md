...
[Preview URLs](https://developers.cloudflare.com/workers/configuration/previews/) are enabled in `wrangler.jsonc` (`preview_urls = true`). The generic `workers.dev` route stays off (`workers_dev = false`); only branch/commit preview URLs are used for QA.
...
| ------------------------------------------------------------------- | ---------------- | -------------------------- |
| `src/`, `public/`, `wrangler.jsonc`, `package.json`, Vite/TS config | Yes              | Yes                        |
